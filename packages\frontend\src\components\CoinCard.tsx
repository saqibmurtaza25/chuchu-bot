...
import { AggregatedSymbolState, CandidateLifecycle } from '@athena/shared';
...
export const getRsiZoneMeta = (rsi: number): RsiZoneMeta => {
  ...
  if (rsi >= 55) {
    return {
      colorClass: 'bg-athena-panel text-slate-300 border border-athena-border font-medium',
      status: 'Bullish',
      badgeLabel: 'Bullish'
    };
  }
  if (rsi >= 45) {
    return {
      colorClass: 'bg-athena-panel text-slate-400 border border-athena-border font-normal',
      status: 'Neutral',
      badgeLabel: 'Neutral'
    };
  }
...
const LIFECYCLE_STYLES: Record<CandidateLifecycle, string> = {
  ...
  QUALIFIED:  'bg-cyan-500/15 text-athena-cyan border-athena-cyan/50',
...
export const CoinCard: React.FC<CoinCardProps> = ({ state }) => {
  const { setSelectedSymbol, setActivePage, submitOrder, focusedSymbol, setFocusedSymbol } = useAthenaStore();
  ...
  return (
    <div className="glass-panel rounded-xl p-3.5 border border-athena-border/80 hover:border-athena-cyan/40 transition-all duration-200 shadow-lg flex flex-col justify-between space-y-2 font-sans text-athena-text">
      {/* Header: Symbol & Price */}
      <div className="flex items-center justify-between pb-1.5 border-b border-athena-border">
        <div>
          <div className="flex items-center space-x-2">
            <span className="text-base font-bold text-athena-text tracking-wide">{symbol}</span>
            <button
              onClick={async (e) => {
                e.stopPropagation();
              ...
              className="p-0.5 hover:text-athena-cyan transition-colors shrink-0"
              title={focusedSymbol === symbol ? "Remove from Sniper Mode" : "Activate Sniper Mode"}
            >
              <Star className={`w-3.5 h-3.5 ${focusedSymbol === symbol ? 'text-athena-cyan fill-athena-cyan' : 'text-athena-muted'}`} />
            </button>
            <span className="text-[10px] px-2 py-0.5 rounded bg-athena-panel text-athena-cyan font-semibold">PERP</span>
            {state.lifecycle && (
          ...
          <div className="text-[10px] text-athena-muted num-font flex items-center space-x-1.5 mt-0.5">
            <span>Last Tick: <strong className="text-white font-mono">{lastTickFormatted}</strong></span>
          </div>
        </div>

        <div className="text-right">
          <div className={`text-lg font-extrabold num-font ${isBuy ? 'text-athena-green' : 'text-athena-red'}`}>
            ${price > 0 ? formatPrice(price) : '---'}
          </div>
          <div className="text-[10px] text-athena-muted num-font">
            VWAP: <span className="text-athena-text">${formatPrice(vwap)}</span> | POC: <span className="text-athena-text">${formatPrice(pocPrice)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-athena-bg/80 p-1.5 rounded-lg border border-athena-border/60 text-xs space-y-1">
        <div className="flex items-center justify-between text-[10px] text-athena-muted font-extrabold tracking-wider uppercase">
          <span>TIMEFRAME RSI MATRIX</span>
          <span className="text-[9px] text-athena-cyan">Overbought / Oversold</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs border-t border-athena-border/50 pt-1.5 num-font">
        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">W%R 200 MTF Avg:</span>
          <span className="font-semibold text-emerald-400 font-bold">{williamsR200Mtf}%</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Funding Rate:</span>
          <span className={`font-semibold ${funding >= 0 ? 'text-athena-green' : 'text-athena-red'}`}>{funding >= 0 ? `+${funding.toFixed(4)}%` : `${funding.toFixed(4)}%`}</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Open Interest:</span>
          <span className="font-semibold text-athena-text">{openInterest.toLocaleString()} <span className="text-[10px] text-athena-green">({oiDelta >= 0 ? `+${oiDelta}%` : `${oiDelta}%`})</span></span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">CVD Delta:</span>
          <span className={`font-semibold ${cvd >= 0 ? 'text-athena-green' : 'text-athena-red'}`}>
            {cvd >= 0 ? `+$${cvdMb}M` : `-$${Math.abs(Number(cvdMb))}M`}
          </span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">24h Volume:</span>
          <span className="font-semibold text-athena-text">${(volume / 1_000_000).toFixed(1)}M</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Regime:</span>
          <span className="font-semibold text-athena-cyan text-[11px] truncate max-w-[90px] font-sans">{regime}</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Orderbook:</span>
          <span className={`font-semibold ${buyerPct >= 50 ? 'text-athena-green' : 'text-athena-red'}`}>
            Buyers {buyerPct.toFixed(0)}%
          </span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Liquidity Sweeps:</span>
          <span className={`font-semibold font-sans ${sweepDetected ? 'text-athena-green animate-pulse' : 'text-athena-muted'}`}>
            {sweepDetected ? 'DETECTED' : 'NONE'}
          </span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Whale Activity:</span>
          <span className={`font-semibold font-sans ${whaleActivity ? 'text-athena-purple font-bold' : 'text-athena-muted'}`}>
            {whaleActivity ? 'ACTIVE' : 'QUIET'}
          </span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Spoofing Prob:</span>
          <span className={`font-semibold ${spoofingProb > 20 ? 'text-athena-red' : 'text-athena-muted'}`}>{spoofingProb}%</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">ATR (14):</span>
          <span className="font-semibold text-athena-text">${atr.toFixed(2)}</span>
        </div>

        <div className="flex justify-between">
          <span className="text-athena-muted font-sans">Risk Level:</span>
          <span className={`font-bold font-sans ${riskLevel === 'EXTREME' ? 'text-athena-red' : riskLevel === 'HIGH' ? 'text-athena-yellow' : 'text-athena-green'}`}>
            {riskLevel}
          </span>
        </div>
      </div>
