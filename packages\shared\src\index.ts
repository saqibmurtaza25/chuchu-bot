...
export interface SignalResult {
  ...
  confidence: number;
  hunterScore?: number; // Added for ATHENA v2.0
  setupQuality?: number; // Added for ATHENA v2.0
  entryPrice: number;
  stopLoss: number;
  takeProfit: number;
  ...
  reasons: string[];
  modifiers?: { name: string; value: number }[]; // Added for ATHENA v2.0 explainability
  timestamp: number;
}
