...
import { DiscoveredCoin, HeatCandidate, PrioritizedCandidate, CandidateLifecycle } from '@athena/shared';
...
const LIFECYCLE_STYLES: Record<CandidateLifecycle, string> = {
  ...
  QUALIFIED:  'bg-cyan-500/15 text-cyan-300 border-athena-cyan/50',
...
const LifecycleBadge: React.FC<{ lifecycle?: CandidateLifecycle }> = ({ lifecycle }) => {
  if (!lifecycle) return <span className="text-athena-muted">—</span>;
  return (
...
const TagBadge: React.FC<{ tag: string }> = ({ tag }) => {
  ...
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded border font-semibold tracking-wide ${styles[tag] || 'bg-athena-panel text-athena-muted border-athena-border'}`}>
      {labels[tag] || tag}
    </span>
  );
};
...
const HeatZoneBadge: React.FC<{ zone: string; confirmed: boolean }> = ({ zone, confirmed }) => {
  ...
  return (
    <div className="flex items-center space-x-1.5">
      <span className={`text-[11px] px-2.5 py-0.5 rounded border font-bold uppercase tracking-wider ${styles[zone] || 'bg-athena-panel text-athena-muted border-athena-border'}`}>
        {zone.replace('_', ' ')}
      </span>
      {confirmed && (
        <span className="text-[10px] px-1.5 py-0.5 rounded border bg-cyan-500/10 text-athena-cyan border-athena-cyan/40 font-bold">
          WMR✓
        </span>
      )}
    </div>
...
const DiscoveryTab: React.FC<{ coins: DiscoveredCoin[] }> = ({ coins }) => (
  <div className="glass-panel rounded-xl overflow-x-auto border border-athena-border shadow-xl">
    <table className="w-full text-left border-collapse text-xs font-sans">
      <thead>
        <tr className="bg-athena-card/90 border-b border-athena-border text-athena-muted font-bold uppercase text-[11px] tracking-wider">
      ...
      <tbody className="divide-y divide-athena-border/40 text-athena-text">
        {coins.length === 0 ? (
          <tr>
            <td colSpan={7} className="py-12 text-center text-athena-muted font-medium text-sm">
              Initializing 4-Stage Discovery Pipeline... (Auto-refreshes every 30s)
            </td>
          </tr>
        ) : (
          coins.map((coin) => (
            <tr key={coin.symbol} className="hover:bg-athena-panel/60 transition-all duration-150">
              <td className="py-3.5 px-5 font-bold text-athena-text text-sm flex items-center space-x-2">
                <span>{coin.symbol}</span>
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-athena-panel border border-athena-border text-athena-cyan font-semibold">PERP</span>
              ...
              <td className="py-3.5 px-5 font-semibold text-athena-text text-sm num-font">
                ${formatPrice(coin.lastPrice)}
              </td>
              <td className={`py-3.5 px-5 font-bold text-sm num-font ${coin.priceChangePercent24h >= 0 ? 'text-athena-green' : 'text-athena-red'}`}>
              ...
              <td className="py-3.5 px-5 font-medium text-athena-muted">
...
const HeatTab: React.FC<{ candidates: HeatCandidate[] }> = ({ candidates }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-athena-card p-3 rounded-xl border border-athena-border">
        <div className="flex items-center space-x-2">
          <Filter className="w-4 h-4 text-athena-cyan" />
          <span className="text-xs font-bold text-athena-text uppercase tracking-wider">FILTER BY TIMEFRAME RSI:</span>
        </div>
        <div className="flex items-center space-x-1">
          {(['ALL', '5m', '15m', '1H', '4H', '12H'] as const).map(tf => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-extrabold transition-all ${selectedTf === tf ? 'bg-athena-cyan text-black shadow-[0_0_10px_rgba(0,240,255,0.3)]' : 'bg-athena-panel text-athena-muted hover:text-athena-text'}`}
            >
              {tf}
            </button>
          ))}
        ...
        <table className="w-full text-left border-collapse text-xs font-sans">
          ...
          <tbody className="divide-y divide-athena-border/40 text-athena-text">
            {filteredCandidates.length === 0 ? (
              <tr>
                <td colSpan={13} className="py-12 text-center text-athena-muted font-medium text-sm">
              ...
              filteredCandidates.map((c) => {
                ...
                return (
                  <tr key={c.symbol} className="hover:bg-athena-panel/60 transition-all duration-150">
                    <td className="py-3.5 px-4 font-bold text-athena-text text-sm">
                      <div className="flex items-center space-x-2">
                        <span>{c.symbol}</span>
                        <span className="text-[10px] font-mono text-athena-yellow font-extrabold bg-athena-yellow/10 px-1.5 py-0.5 rounded">
                    ...
                    {[
                      ...
                      <td key={cell.tf} className="py-3.5 px-2 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs num-font font-bold ${
                          ...
                          'bg-athena-panel text-slate-300 border border-athena-border'
                        }`}>
                          {cell.val}%
                        </span>
                      </td>
...
const SignalsTab: React.FC<{
  ...
  <div className="space-y-4 font-sans">
    {signals.length === 0 ? (
      <div className="glass-panel rounded-xl border border-athena-border p-12 text-center text-athena-muted space-y-2">
        <div className="text-2xl font-bold text-athena-text">🎯 No High-Probability Signals Currently</div>
        <p className="text-xs text-athena-muted max-w-md mx-auto">
          ATHENA's institutional signal engine filters noise strictly — 0 to 5 high-conviction scalping setups expected per scan cycle.
        </p>
      </div>
    ) : (
      signals.map((s) => {
        ...
        return (
          <div
            key={s.symbol}
            className={`glass-panel rounded-xl border p-5 transition-all shadow-lg ${isBuy ? 'border-athena-green/50 bg-athena-green/5' : isSell ? 'border-athena-red/50 bg-athena-red/5' : 'border-athena-border'}`}
          >
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <span className="font-extrabold text-athena-text text-lg tracking-wide">{s.symbol}</span>
                <span className="text-base" title={s.priorityLabel}>{s.priorityStars}</span>
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded bg-athena-panel text-athena-text border border-athena-border">
                  {s.priorityLabel}
                </span>
              ...
              <div className="flex items-center space-x-3">
                <div className="flex flex-col items-end mr-2">
                  <span className="text-[10px] text-athena-muted font-bold tracking-widest uppercase">RATE / DELAY</span>
                  <div className="flex items-center space-x-1.5">
                    <span className="font-mono text-sm text-athena-text font-bold">${formatPrice(s.state?.lastTick?.price || 0)}</span>
                ...
                <span className={`text-sm font-extrabold px-3.5 py-1 rounded border uppercase tracking-wider ${isBuy ? 'bg-athena-green/20 text-athena-green border-athena-green/50' : isSell ? 'bg-athena-red/20 text-athena-red border-athena-red/50' : 'bg-athena-panel text-athena-muted border-athena-border'}`}>
                  {sig?.signal || 'NEUTRAL'}
                </span>
                <div className="flex flex-col text-right text-[10px] sm:text-xs font-bold num-font text-athena-cyan">
                  <span>Hunter: {sig?.hunterScore || 0}</span>
                  <span className="text-athena-yellow">Quality: {sig?.setupQuality || 0}/100</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 mb-4 text-xs">
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">RSI (5m)</div>
                <div className={`font-extrabold text-sm num-font ${s.rsi5m > 70 ? 'text-rose-400' : s.rsi5m < 30 ? 'text-emerald-400' : 'text-athena-text'}`}>{s.rsi5m}</div>
              </div>
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">WMR (200)</div>
                <div className={`font-extrabold text-sm num-font ${s.wmr200 > -20 ? 'text-rose-400' : s.wmr200 < -80 ? 'text-emerald-400' : 'text-athena-text'}`}>{s.wmr200}</div>
              </div>
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">Composite Score</div>
                <div className="font-extrabold text-sm text-athena-cyan num-font">{sig?.compositeScore ?? '---'}</div>
              </div>
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">Priority Rank</div>
                <div className="font-extrabold text-sm text-athena-text">P{s.priority}</div>
              </div>
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">Stop Loss</div>
                <div className="font-extrabold text-sm text-athena-red num-font">${sig?.stopLoss ? formatPrice(sig.stopLoss) : '---'}</div>
              </div>
              <div className="bg-athena-panel/80 rounded-lg p-2.5 border border-athena-border/60">
                <div className="text-athena-muted font-medium text-[11px] mb-1">Take Profit</div>
                <div className="font-extrabold text-sm text-athena-green num-font">${sig?.takeProfit ? formatPrice(sig.takeProfit) : '---'}</div>
              </div>
            </div>
            ...
            {s.state && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 bg-slate-950/70 p-2 rounded-lg border border-slate-700/60 text-[11px] my-2.5 num-font font-sans">
                ...
                <div className="p-1.5 bg-slate-900/90 rounded border border-slate-700/60">
                  ...
                  <div className="font-extrabold text-xs text-athena-cyan truncate">{s.state.regime?.regime || 'MEAN_REVERTING'}</div>
                </div>
                <div className="p-1.5 bg-slate-900/90 rounded border border-slate-700/60">
            ...
            {sig?.reasons && sig.reasons.length > 0 && (
              <div className="text-xs text-athena-muted space-y-1 mb-3 font-sans bg-athena-card/50 p-2.5 rounded-lg border border-athena-border/50">
                ...
                {sig.reasons.slice(0, 4).map((r, i) => (
                  <div key={i} className="text-slate-300 flex items-center space-x-1.5">
                    <span className="text-athena-cyan font-bold">•</span>
                    <span>{r}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-athena-green/20 text-athena-green border border-athena-green/40 hover:bg-athena-green/30 font-bold flex items-center space-x-2 text-xs transition-all shadow-md"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-athena-red/20 text-athena-red border border-athena-red/40 hover:bg-athena-red/30 font-bold flex items-center space-x-2 text-xs transition-all shadow-md"
                >
              ...
              <span className="text-xs font-semibold text-athena-muted">Risk-Reward Ratio: 1:{sig?.riskRewardRatio || 2.5}</span>
            </div>
          </div>
        );
      })
...
export const ScannerPage: React.FC = () => {
  const { states, setSelectedSymbol, setActivePage, submitOrder, pipeline, pipelineLoading, fetchPipeline } = useAthenaStore();
  ...
  const tabs = [
    ...
    },
    {
      ...
      activeBg: 'border-athena-cyan/60 bg-athena-cyan/10',
    },
    {
      id: 'all' as ScannerTab,
      label: '⚙ All Tracked',
      count: symbolList.length,
      color: 'text-athena-muted',
      activeBg: 'border-athena-border bg-athena-panel',
    },
  ];

  return (
    <div className="p-6 space-y-6 font-sans antialiased text-athena-text">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-athena-text tracking-tight flex items-center space-x-2.5">
            <Cpu className="w-6 h-6 text-athena-cyan" />
            <span>ATHENA HUNT ENGINE</span>
          </h1>
        ...
        <div className="flex items-center space-x-3 text-xs font-semibold">
          {meta && (
            <>
              <span className="text-orange-400 bg-orange-500/10 px-2.5 py-1 rounded border border-orange-500/20">Stage 1: <strong className="text-athena-text num-font">{meta.discoveredCount}</strong></span>
              <span className="text-rose-400 bg-rose-500/10 px-2.5 py-1 rounded border border-rose-500/20">Stage 2: <strong className="text-athena-text num-font">{meta.heatCount}</strong></span>
              <span className="text-athena-cyan bg-cyan-500/10 px-2.5 py-1 rounded border border-cyan-500/20">Signals: <strong className="text-athena-text num-font">{meta.signalCount}</strong></span>
            </>
          )}
          <button
            onClick={fetchPipeline}
            disabled={pipelineLoading}
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border border-athena-border bg-athena-card text-athena-muted hover:text-athena-text hover:border-athena-cyan/50 transition-colors shadow-sm text-xs font-semibold"
          >
      ...
      {/* Tabs */}
      <div className="flex items-center space-x-2 border-b border-athena-border pb-2">
        {tabs.map(tab => (
          <button
            key={tab.id}
            ...
            className={`flex items-center space-x-2 px-5 py-2.5 rounded-t-lg border-b-2 text-xs font-extrabold tracking-wide transition-all duration-150 ${
              activeTab === tab.id
                ...
                : 'border-transparent text-athena-muted hover:text-athena-text'
            }`}
          >
            <span>{tab.label}</span>
            <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold num-font ${activeTab === tab.id ? 'bg-athena-card text-athena-text' : 'bg-athena-panel text-athena-muted'}`}>
              {tab.count}
            </span>
          </button>
        ))}
      ...
      {activeTab === 'all' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1.5 bg-athena-card p-1 rounded-lg border border-athena-border text-xs">
              <span className="text-athena-muted text-[11px] font-bold px-2">SORT BY:</span>
              {(['candidate','heat','ai','volume'] as const).map(s => (
                <button
                  key={s}
                  onClick={() => setSortBy(s)}
                  className={`px-2.5 py-1 rounded text-[11px] font-bold uppercase transition-all ${sortBy === s ? 'bg-athena-cyan/20 text-athena-cyan' : 'text-athena-muted hover:text-athena-text'}`}
                >
                  {s}
                </button>
              ))}
          ...
          <div className="glass-panel rounded-xl overflow-x-auto border border-athena-border shadow-xl">
            <table className="w-full text-left border-collapse text-xs font-sans">
              ...
              <tbody className="divide-y divide-athena-border/40 text-athena-text">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-athena-muted">
                  ...
                  filtered.map(state => {
                    ...
                    return (
                      <tr key={state.symbol} className="hover:bg-athena-panel/60 transition-all duration-150">
                        <td className="py-3.5 px-5 font-bold text-athena-text text-sm flex items-center space-x-2">
                          <span>{state.symbol}</span>
                          <span className="text-[10px] px-1.5 py-0.2 rounded bg-athena-panel text-athena-cyan font-semibold">PERP</span>
                        </td>
                        <td className="py-3.5 px-5 font-bold text-athena-text text-sm num-font">
                          ${price > 0 ? formatPrice(price) : '---'}
                        </td>
                        <td className="py-3.5 px-5 text-athena-yellow font-extrabold num-font">
                          <div className="flex items-center space-x-1">
                            <Flame className="w-3.5 h-3.5" />
                            <span>{heat}</span>
                          </div>
                        </td>
                        <td className="py-3.5 px-5 text-athena-cyan font-extrabold num-font">{candidate}</td>
                        <td className="py-3.5 px-5 text-athena-purple font-extrabold num-font">{ai}</td>
                        <td className="py-3.5 px-5">
                          <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-athena-panel border border-athena-border text-athena-cyan">
                            {regime}
                          </span>
                        </td>
                        <td className={`py-3.5 px-5 font-extrabold num-font ${obi >= 0 ? 'text-athena-green' : 'text-athena-red'}`}>
                          {obi >= 0 ? '+' : ''}{(obi * 100).toFixed(0)}%
                        </td>
                        <td className="py-3.5 px-5 text-right">
                          <div className="flex items-center justify-end space-x-2">
                            <button onClick={() => handleTrade(state.symbol, 'BUY')} className="px-3 py-1 rounded-lg bg-athena-green/20 text-athena-green border border-athena-green/40 hover:bg-athena-green/30 font-bold flex items-center space-x-1 text-xs">
                              <ArrowUpRight className="w-3.5 h-3.5" /><span>BUY</span>
                            </button>
                            <button onClick={() => handleTrade(state.symbol, 'SELL')} className="px-3 py-1 rounded-lg bg-athena-red/20 text-athena-red border border-athena-red/40 hover:bg-athena-red/30 font-bold flex items-center space-x-1 text-xs">
